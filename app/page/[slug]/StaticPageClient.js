...
export default function StaticPageClient({ slug }) {
  ...
  if (loading) {
    const preTheme = typeof window !== 'undefined' ? (localStorage.getItem('shindora-theme') || 'dark') : 'dark';
    return (
      <div className={`min-h-screen flex items-center justify-center transition-colors duration-200 ${
        preTheme === 'dark' ? 'bg-[#07080f] text-white' : 'bg-white text-slate-900'
      }`}>
        <div className={`w-8 h-8 border-4 border-t-transparent rounded-full animate-spin mr-3 ${
          preTheme === 'dark' ? 'border-cyan-500' : 'border-cyan-600'
        }`} />
        <span className={`text-xs font-bold uppercase tracking-widest animate-pulse ${
          preTheme === 'dark' ? 'text-cyan-400' : 'text-slate-800'
        }`}>Memuat Dokumen Halaman...</span>
      </div>
    )
  }
      ...
      <main className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        {pageData ? (
          <article className="space-y-4">
            <h1 className={`text-2xl md:text-3xl font-black border-b pb-4 ${
              theme === 'dark' ? 'text-white border-slate-500/10' : 'text-slate-900 border-slate-200'
            }`}>
              {pageData.title}
            </h1>
            <div 
              className={`prose text-xs md:text-sm leading-relaxed space-y-4 ${
